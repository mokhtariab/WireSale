...
namespace WireSales_Prj
{
    public partial class StartHM_frm : Form
    {
        ...
        private void StartHM_frm_Load(object sender, EventArgs e)
        {
            ...
            //MessageBox.Show("قفل و کانکشن");
            Function_Cls.TinySetFunc(TinyWire, Global_Cls.Lock_UserID, Global_Cls.Lock_SerialNo, Global_Cls.Lock_SpecialID);
            SetLanguageProgram();
        }
        ...
        private void StartHM_frm_Shown(object sender, EventArgs e)
        {
            ...
            //Function_Cls.RunScript();
        }
        
        private void CheckOrInstallDB()
        {
            Global_Cls.ConnectionStr = "Data Source=.\\sqlexpress;Initial Catalog=WireSales;Integrated Security=True;";
            //Global_Cls.ConnectionStr = "Data Source=.;Initial Catalog=WireSales;Integrated Security=True;";
            ...
            catch (Exception EXstr)
            {
                ...
                SqlConnection SqlConn1 = new SqlConnection(Global_Cls.ConnectionDef + "Initial Catalog=master;");
                SqlCommand SqlCmd = new SqlCommand();
                SqlCmd.Connection = SqlConn1;
                SqlCmd.CommandText = "USE [master]" +
                    "CREATE DATABASE [WireSales] ON " +
                    "( FILENAME = N'" + Application.StartupPath + @"\ApplicationData\Data\WireSales.mdf' )," +
                    "( FILENAME = N'" + Application.StartupPath + @"\ApplicationData\Data\WireSales_log.ldf' )" +
                    " FOR ATTACH ";
                try
                {
                    SqlConn1.Open();
                ...
                {
                    Global_Cls.Message_Sara(0, Global_Cls.MessageType.SError, false, "اتصال دیتا: نصب برنامه به درستی صورت نگرفته است! لطفا تماس بگیرید.", ex.Message);
                    Application.Exit();
                }
            }
